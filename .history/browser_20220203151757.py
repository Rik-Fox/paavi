...
class Browser(object):
    def __init__(self, envision_port, init_pause_state=True):
        ...
        self._driver.get(f"http://localhost:{self._port}")
    ...
    def get_paused(self):
        for entry in self._driver.get_log("browser"):
            if entry["source"] == "console-api":
                ...
                if (
                    entry["message"]
                    == f"http://localhost:{self._port}/main.js 384:88417 true"
                ):
                    self._paused = True
                    return self._paused
                elif (
                    entry["message"]
                    == f"http://localhost:{self._port}/main.js 384:88417 false"
                ):
                    self._paused = False
                    return self._paused
